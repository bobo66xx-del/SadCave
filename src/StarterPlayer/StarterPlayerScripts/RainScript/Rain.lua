...
function Rain:IsEnabled()
	return not disabled
end

function Rain:SetEnabled(isEnabled, tweenInfo)
	if typeof(isEnabled) ~= "boolean" then
		error("bad argument #1 to 'SetEnabled' (boolean expected, got " .. typeof(isEnabled) .. ")", 2)
	end

	if isEnabled then
		self:Enable(tweenInfo)
	else
		self:Disable(tweenInfo)
	end
end
