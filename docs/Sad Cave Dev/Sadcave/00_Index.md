...
📍 **PROJECT ON HIATUS as of 2026-04-30.** Tyler stepping away from Sad Cave for a while: *"ok wrap up i think im done with this project for a while its become too much for me."* Last shipped state is post-PR-#27 `main` (Brief B medium polish, merged 2026-04-30 04:38:32 UTC). The session 16 wrap (`07_Sessions/2026-04-30_session_1.md`) covers what happened between PR #27 and the hiatus call: Aura Pass kicked off, broke visually, reverted end-to-end and **scrapped** (not redesigned) per Tyler; Avalog removal feasibility investigated and **blocked pending Tyler's keep-or-remove call on the avatar editor itself** (filed in `_Open_Questions.md`). When/if Tyler returns: re-orient via `AGENTS.md`, then triage from the queued items below. Nothing is in flight; no branches lingering; GitHub `list_branches` confirms only `main` remains.

Active focus (queued, not in flight):
...
- [ ] **Avalog cleanup decision** — deferred pending Tyler's keep-or-remove call on the avatar editor. Investigation complete (2026-04-30 session_1), full context in [[09_Open_Questions/_Open_Questions]] § Active. TL;DR: `Workspace.Avalog` is a third-party avatar editor framework with one HARD external dep (`FavoritePromptPersistence` uses Avalog's `PlayerDataStore` for the `FavoritePromptShown` flag) and one soft entry-point (Avatar TextButton calls `Avalog:ToggleOpen()`). Removal is feasible but requires migrating the FavoritePrompt persistence first — a small but non-trivial PR. KEEP → no-op, leave alone. REMOVE → design + ship the persistence migration before any deletion.
- [ ] **Production cutover brief for Title v2** — write later, after the testing place soaks for a while. Migration runtime-verified by PR #17; cutover brief still needs a rollback plan, soak-period monitoring, and Tyler's "okay, push to prod" signal per `01_Vision/Environments.md`. The `LoadAndMigrateForUserId` helper is available as a tooling surface if a one-off mass-migration pass becomes useful.
...
- [[06_Codex_Plans/2026-04-29_Nametag_Aura_Pass_v1]] — ⚫ Scrapped 2026-04-30 (was 🔵 Queued from session 15). Codex's `codex/nametag-aura-pass` commit `63090ce` implemented this brief faithfully but the dark-cushion-as-legibility-base read as a giant dark slab in playtest — root cause was the design call (dark plate underneath text), not implementation. Tyler scrapped rather than redesigned ("i dont think we can do it. maybe another time"). Branch deleted local + remote, working tree reverted to pre-aura `NameTagEffectController.client.lua`. Spec kept as historical record in `NameTag_Status.md` § "Aura Pass — Stillness Bloom" for possible future revival; recommend a different visual direction (B Inscribed Plaque / C Tethered Presence / D Atmospheric Whisper) over re-trying Direction A. See `_Decisions.md` 2026-04-30 entry + `07_Sessions/2026-04-30_session_1.md`.
