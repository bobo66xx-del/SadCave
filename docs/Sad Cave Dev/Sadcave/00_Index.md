...
- [ ] **AchievementTracker — Codex-implemented, awaiting merge.** Brief shipped 2026-04-29 (`06_Codex_Plans/2026-04-29_AchievementTracker_v1.md`). Codex pushed `codex/achievement-tracker` (head `2df4378`); Claude review passed playtest verdict "safe to merge" — four achievements (`came_back`, `keeps_coming_back`, `part_of_the_walls`, `one_of_us`) verified firing on Tyler's join, gating works for `heard_them_all` + `day_one`, no errors. Six interactive achievements (`said_something`, `sat_down`, `left_a_mark`, `knows_every_chair`, `up_too_late`, `fell_asleep_here`) read clean in diff but Tyler can verify them in normal play. **Pending:** Tyler merges `codex/achievement-tracker` into main; next-session integration backfills the change log with `HH:MM UTC` + PR #.
