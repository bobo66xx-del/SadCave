...
**Title v2 player-facing surface is feature-complete on polish + desktop refinement, with the Achievement category live.** PR #12 (06:10 UTC) shipped the data + display layer. PR #13 (07:22 UTC) cleaned up the architecture (TitleService as ModuleScript). PR #14 (08:31 UTC) added the manual-equip surface. PR #17 (2026-04-29 03:08 UTC) runtime-verified the v1 → v2 migration. PR #20 (2026-04-29 08:52 UTC) activated the Achievement title category — 12 titles live. PR #23 (2026-04-29 10:48 UTC) shipped the polished menu + nametag visual pass. **PR #25 (2026-04-29 14:01 UTC) shipped the Desktop Refinement Pass — Brief A — via two-iteration same-branch loop**: per-client desktop sizing (BillboardGui 280×80, title 16pt, name 25pt), stillness fade tied to "presence rewards stillness" (title fades after sustained running > 2.0s, restores on stop), distance fade (title softens 20-40 studs, BillboardGui hides at MaxDistance 50 via client-side override applied on both platforms), per-tab notification dot when titles unlock, per-row notification dots in TitleMenu (clear on close), edge tab cross-fades out as drawer opens (re-click-tab close path replaced by three remaining: outside-click, ESC, internal `x`). Eighteen PRs shipped 2026-04-27 → 2026-04-29 (latest: PR #25 desktop refinement, 14:01:58 UTC).

Active focus:
- [ ] **Title v2 — Brief B (medium polish) deferred / pending real-play observation.** PR #25 shipped Brief A's full bundle including the bigger title size and tighter visibility range; some Brief B items (background-aware stroke tuning, breath between rows) may be partly addressed by those changes. Sit with the new state for a session or two of normal play, then decide whether Brief B is still needed in full, partial, or at all. Held items (title-row letter tracking, first-time tab pulse) revisit only if real-play data shows need.
