...
- AFK-vs-sitting design call (caught Cowork session 5 walkthrough): `PresenceTick.GetTickAmount` checks `state.isAFK` first and returns `PRESENCE_AFK_XP=3` even when `state.seatedAt` is set. Observed live — Tyler seated on `Workspace.SeatMarkers.Seat`, leaderstats XP ticked +3 instead of +20 because the Studio window had lost focus to Cowork, triggering `AfkDetector` → `AfkEvent:FireServer(true)`. Three branches Tyler needs to pick from before Codex implements: **Branch A — prefer-seated-over-AFK**: in `PresenceTick.GetTickAmount`, check `state.seatedAt` (with elapsed ≥ threshold) before `state.isAFK`. A seated player keeps sitting rate even with focus lost. Lowest-friction for the Sad Cave tone (presence-rewarding, quiet) but technically lets a window-buried player farm seat XP. **Branch B — tighten AFK detection**: in `AfkDetector.client.lua`, only fire AFK if focus-loss AND no input for N seconds. Same end behavior in many cases but more state to maintain. **Branch C — leave as-is**: AFK overrides sitting by design. Document in `02_Systems/XP_Progression.md` and add a Decision entry. Brief at [[../06_Codex_Plans/2026-04-27_XP_Testing_Place_Bug_Sweep_v1]] is queued and waits on this call.
