...
**Status:** 🟡 Building — Title v2 player-facing surface is feature-complete on polish + desktop refinement, with the Achievement category live. MVP-1 shipped via PR #12 (2026-04-28 06:10 UTC, data + display layer). PR #13 (2026-04-28 07:22 UTC) cleaned up the TitleService architecture (ModuleScript pattern, respawn-resilient level watcher). MVP-2 shipped via PR #14 (2026-04-28 08:31 UTC, manual equip + placeholder menu + production-cutover migration). PR #17 (2026-04-29 03:08 UTC) runtime-verified the v1 → v2 migration. PR #20 (2026-04-29 08:52 UTC) activated the Achievement title category — 12 titles light up. PR #23 (2026-04-29 10:48 UTC) shipped the polished menu + nametag visual pass. **PR #25 (2026-04-29 14:01 UTC) shipped the Desktop Refinement Pass — Brief A — via two-iteration same-branch loop**: per-client desktop sizing (BillboardGui 280×80, title 16pt, name 25pt), stillness fade tied to "presence rewards stillness" (title fades after sustained running > 2.0s), distance fade (title softens 20-40 studs, BillboardGui hides at MaxDistance 50 via client override), per-tab notification dot, per-row notification dots in TitleMenu, edge tab cross-fades with drawer open/close. **Brief B (medium polish) 🟢 Shipped** — PR #27 merged 2026-04-30 04:38:32 UTC, branch `codex/title-polish-brief-b`, head `d3c6958`. Two non-blocking carry-forwards parked (Step 0 bright-background test skipped — effectively moot once Aura Pass cushion ships; recess fade preemptive — one-line revert in any future polish touch). Letterspacing item from Brief B's "held" list folded into Aura Pass. **Nametag Aura Pass 🔵 Queued** (Stillness Bloom design specced 2026-04-29 session_5 — the biggest visual upgrade since the polished pass; cushion + edge bloom + outer halo + breath + stillness-responsive bloom + tint bleed + letterspacing + choreography; canonical design at [[NameTag_Status]] § "Aura Pass — Stillness Bloom"; brief at [[../06_Codex_Plans/2026-04-29_Nametag_Aura_Pass_v1]]). Player-facing surface complete on polish + desktop refinement; remaining v2 work is category activations (Presence next, then Exploration + Seasonal) and production cutover.
...
## Desktop Refinement Pass — Sizing + Presence Behaviors (designed 2026-04-29, 🟢 Shipped, Iteration 1 final)

**Status:** 🟢 Shipped — PR #25 merged 2026-04-29 14:01:58 UTC, branch `codex/title-tag-tab-desktop-refinement`, head `9bca238`. Two-iteration same-branch loop: initial pass (`4aef1de`) matched the original brief, Tyler reviewed live and asked for adjustments, iteration-1 (`9bca238`) incorporated five deltas (proportional size bump to title 16 / name 25 / 280×80, stillness threshold 1.0s → 2.0s, distance fade halved with MaxDistance halved to 50, tab fade-out-on-open replacing re-click-tab close path, per-row notification dots in TitleMenu with clear-on-close). Both Claude reviews ran clean, Tyler merged. Brief at `06_Codex_Plans/2026-04-29_Title_Tag_Tab_Desktop_Refinement_v1.md` (amended inline + § Iteration 1 at end). Brief B (medium polish — stroke tuning, row breath, hover affordance, edge anchor, drawer-dim) deferred until Tyler plays the new state in real conditions and confirms what (if anything) still needs work. Two items held entirely (title-row letter tracking, first-time tab pulse) — revisit only if real-play data shows need.
...
### Brief A — high-impact bundle (🟢 Shipped via PR #25, Iteration 1 final)

The five changes that delivered the most felt difference. Shipped via two-iteration same-branch loop.

#### 1. Desktop nametag size bumps (Iteration 1: proportional bump up)

The BillboardGui and its two TextLabels scale up on desktop only. Iteration 1 amended initial values (title 14 / name 22 / 240×64) to a larger proportional bump (title 16 / name 25 / 280×80).
...
| `TitleLabel.Position.Y.Offset` | 0 | 0 (still top) |
| `NameLabel.TextSize` | 16 | 25 |
| `NameLabel.Size.Y.Offset` | 28 | 41 |
| `NameLabel.Position.Y.Offset` | 19 | 28 |

Total desktop stack: 22px title + ~6px gap + 41px name + slack ≈ 80px. Width bumps 200 → 280 to give long titles like "always was here" (15 chars) room without clipping at the new larger size.

`StudsOffset Vector3.new(0, 3, 0)` unchanged. **`MaxDistance` overridden client-side from 100 → 50** — see § Distance fade.
...
#### 3. Stillness fade on the title row (Iteration 1: longer running threshold)
...
- **Trigger threshold:** `humanoidRootPart.AssemblyLinearVelocity.Magnitude > 10 stud/s` sustained for `≥ 2.0s` *(Iteration 1: was 1.0s)* → start fading title `TextTransparency` from baseline (~0.25) to fully transparent (1.0) over 0.6s, ease-out sine.
- **Return threshold:** velocity drops `< 10 stud/s` sustained for `≥ 0.4s` → fade back to baseline over 0.6s, ease-out sine.
- **Hysteresis** matters — without sustained-window thresholds, the title would flicker during normal walking pauses. The 2.0s out / 0.4s back asymmetry biases toward "presence-rewarding": you have to actually run sustained — not just walk briskly across a small area — before the title commits to fading; but a brief stop returns it quickly. Iteration 1 raised the bar because the original 1.0s felt too aggressive in playtest (title was fading during normal traversal pauses, not just sustained running).
...
#### 4. Distance fade on the title row (Iteration 1: halved bands + halved MaxDistance)

Title row softens as the viewer's camera moves further from the viewed character. Reinforces "presence has a tighter range." Name row stays at its baseline opacity. Iteration 1 halved both the fade bands and the BillboardGui's `MaxDistance` (the latter as a client-side override, applied on both platforms).

- Camera-to-adornee distance bands:
  - 0–20 studs *(was 0–40)*: title at baseline transparency (~0.25)
  - 20–40 studs *(was 40–80)*: title fades linearly from baseline to 0.85
  - 40–50 studs: title fully transparent (1.0); name still visible at baseline
- BillboardGui itself disappears at **MaxDistance = 50** *(was 100)* via a per-client override on every NameTag (server property stays untouched).
- Update cadence: 10Hz (every 0.1s) is plenty — no need for per-frame. Use `task.wait(0.1)` loop or accumulator on Heartbeat.

Composes with stillness fade by **multiplying** the resulting transparency: a far-away running player's title is fully transparent; a close-and-still player's title is at baseline. Combined with the size bump above, the design intent reads: "bigger nearby, gone sooner at distance" — coherent with the polished pass's tone.
...
#### 7. Tab hide-on-open + reappear-on-close (Iteration 1: new)

When the drawer opens, the edge tab fades to fully invisible at roughly the same pace as the drawer slide-in (~0.2s); when the drawer closes, the tab fades back to its resting state. This drops the "re-click the tab to close" path — three close paths remain (click outside, ESC, internal `x`). Tyler explicitly accepted this trade in iteration 1 review.

Behavior:

- On `OpenRequested.Event` (existing): tween tab `BackgroundTransparency`, label `TextTransparency`, `UIStroke.Transparency`, and `notifyDot.BackgroundTransparency` to 1.0 over 0.2s ease-out sine. Set `button.Active = false` to suppress accidental clicks on the invisible position.
- On `CloseRequested.Event` (existing): tween back to resting values (button 0.25, label 0.4, stroke 0.82) over 0.25s ease-out sine. Set `button.Active = true` immediately so click-responsiveness returns even if visuals are mid-resolve.
- Hover/press handlers gate on `isOpen` — no-op if drawer is open (the tab is supposed to be invisible).
- Quick open-close-open: cancel any in-flight visibility tween before starting the next so transitions don't snap.

The design rationale: while the drawer is open, the tab is redundant signal — the player has clearly found and engaged the menu. A persistent half-visible tab adds visual noise during interaction. Cross-fading with the drawer's slide animation keeps the moment clean.

#### 8. Per-row notification dots in the TitleMenu (Iteration 1: new)

Mirror the tab dot pattern at the row level: when a title unlocks during the session, the row for that title in the menu gets a small warm-grey pulsing dot. Player sees the dots while browsing, dots clear when the menu closes after being open.

Visual:

- Frame named `RowNotifyDot`, child of the row.
- Size 4×4px (smaller than the tab dot's 6×6 — denser context).
- Color `(225, 215, 200)` warm-grey, matches tab dot.
- Anchored at the right edge of the row (`AnchorPoint (1, 0.5)`, `Position (1, -10, 0.5, 0)`).
- `BackgroundTransparency` pulses 0.2 ↔ 0.6 over 2.5s sine cycle.
- `UICorner` full radius for a circle.
- ZIndex one above the row's standard content.

Trigger logic:

- TitleMenuController extends its existing `TitleDataUpdated` listener with the same `firstPayloadReceived` + diff pattern as the tab controller. New IDs in `OwnedTitleIds` join an `unlockedThisSession` set.
- When rendering owned-title rows, attach a `RowNotifyDot` child if the title's ID is in `unlockedThisSession` and not in `dotsClearedFromMenu`.
- Locked rows never get dots (only owned).

Clear logic (Tyler chose Option A — clear on close):

- Dots persist the entire time the menu is open.
- On `CloseRequested.Event` after the menu has been visible: for every titleId currently dotted, fade its dot via 0.4s ease-out sine and mark `dotsClearedFromMenu[titleId] = true`. Don't clear `unlockedThisSession` itself — that drives "is this a fresh row" diffing.
- Subsequent unlocks during the same session still trigger dots for the new IDs only.

The two dot signals split nicely: tab dot says *"you have new things"* (clears on open), row dots say *"specifically these new things"* (clear on close after the player has seen them).

### Brief B — medium polish bundle (🟡 Building)

Written 2026-04-29 session_5 at [[../06_Codex_Plans/2026-04-29_Title_Polish_Brief_B_v1]]. Branch slug `codex/title-polish-brief-b`. Status 🟡 Building — Codex on branch as of session_5 kickoff. Updated from the original "5-item, no file yet" framing: **item 5 (drawer-dim while menu open) is dropped — obsolete per Brief A iteration 1's tab fade-out-on-open** (which goes further than dim and replaces it entirely). Held items (letter tracking, first-time tab pulse) stay held; rationale refreshed at brief time.
...
5. ~~**Drawer-dim while menu open.** When the drawer is showing, the tab is redundant — the player has clearly found it. Drop tab opacity to ~50% while drawer is open; restore on close. Reduces visual noise during menu interaction.~~ — **DROPPED.** Brief A iteration 1's `tab fade-out-on-open` (Step 8b in [[../06_Codex_Plans/2026-04-29_Title_Tag_Tab_Desktop_Refinement_v1]]) takes the tab to fully invisible while the drawer is open, which goes further than "~50% dim." Item 5 is now superseded; if the full fade-out feels too aggressive in real play, the partial-dim variant is a one-line tween-value tweak in `TitlesToggleController.client.lua` (the `OpenRequested.Event` handler), tracked separately if needed.
