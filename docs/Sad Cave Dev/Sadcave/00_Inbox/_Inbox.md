...
- [C] 21:51 — Deleted the duplicate live `StarterPlayerScripts.PromptFavorite` `Script`; Studio now reports one remaining `PromptFavorite`, class `LocalScript`.
- [C] 21:51 — Patched `src/StarterPlayer/StarterPlayerScripts/PromptFavorite.client.lua` with a 10s bounded wait for `FavoritePromptShown` and a tagged graceful-return warning if the remote is missing.
- [C] 21:51 — Playtested: no infinite-yield warning on `FavoritePromptShown`; console showed `FavoritePromptPersistence` creating/using eligibility attributes, `[FavoritePrompt] delayed prompt started FavDelay= 600`, and no new PromptFavorite errors. The known `FavoritePromptPersistence` line-4 error did not reproduce in this run.
